<?php
    ...
    function displayAccountInformation($username, $us_state_abbrevs) {
        try {
            ...
            $accountInformation = $connection->prepare('SELECT FIRSTNAME, LASTNAME, ADDRESS, CITY, STATE, ZIP, PHONE, MEMBER_EMAIL FROM ' . USER_CREDENTIAL_TABLE . ' WHERE MEMBER_EMAIL = :username');
    ...
    function submitAccountInformation($username, $submittedFirstName, $submittedLastName, $submittedAddress, $submittedCity, $submittedState, $submittedZip, $submittedPhone, $submittedEmail) {
        try {
            ...
            $accountInformationUpdate = $connection->prepare('UPDATE ' . USER_CREDENTIAL_TABLE . ' SET FIRSTNAME = :submittedFirstName, LASTNAME = :submittedLastName, ADDRESS = :submittedAddress, City = :submittedCity, State = :submittedState, ZIP = :submittedZip, PHONE = :submittedPhone, MEMBER_EMAIL = :submittedEmail WHERE MEMBER_EMAIL = :username');
